...
// --- THREE.JS BACKGROUND ---
const DynamicSpaceBackground = ({ theme }) => {
    ...
    useEffect(() => {
        if (starsRef.current) {
            starsRef.current.material.color.setHex(THEMES[theme].starColor);
        }
    }, [theme]);

    return (
        <div ref={mountRef} className="fixed inset-0 z-0 bg-[#02020a]">
            <div className={`absolute top-[10%] left-[20%] w-[500px] h-[500px] blur-[150px] rounded-full pointer-events-none transition-all duration-1000 opacity-20 ${THEMES[theme].bgGlow}`} />
        </div>
    );
};
...
export default function IbnayHussain() {
    ...
    return (
        <div className="bg-[#02020a] text-white selection:bg-purple-500/30 min-h-screen font-sans scroll-smooth">
            ...
            <DynamicSpaceBackground theme={currentTheme} />

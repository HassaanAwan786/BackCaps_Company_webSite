...
const CinematicShineBorder = ({ children, theme, compact = false }) => {
    return (
        <div className={`relative p-[3px] group transition-all duration-1000 ${compact ? 'rounded-2xl' : 'rounded-[2rem] sm:rounded-[3.5rem]'} overflow-hidden`}>
            ...
            <div className={`relative z-10 bg-[#02020a]/95 backdrop-blur-3xl transition-all duration-1000 ${compact ? 'rounded-xl p-6 sm:p-8' : 'rounded-[1.75rem] sm:rounded-[3rem] p-6 sm:p-12 lg:p-20'} border border-white/5 shadow-2xl`}>
                <div className={`absolute inset-0 border pointer-events-none transition-colors duration-1000 ${compact ? 'rounded-xl' : 'rounded-[1.75rem] sm:rounded-[3rem]'}`} style={{ borderColor: `${THEMES[theme].primary}20` }} />
                {children}
            </div>
...
const PortfolioFooter = ({ theme }) => {
    return (
        <footer className="relative z-10 py-12 sm:py-20 px-6 sm:px-10 border-t border-white/5 bg-[#02020a]/50 backdrop-blur-md">
            <div className="max-w-8xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8 sm:gap-12">
...
export default function IbnayHussain() {
    ...
    return (
        <div className="bg-[#02020a] text-white selection:bg-purple-500/30 min-h-screen font-sans scroll-smooth">
            ...
            <header className={`fixed top-0 left-0 w-full z-[100] transition-all duration-500 px-4 sm:px-10 ${isScrolled ? 'py-4 bg-[#02020a]/80 backdrop-blur-xl border-b border-white/5' : 'py-6 sm:py-10 bg-transparent mix-blend-difference'}`}>
                <div className="max-w-8xl mx-auto w-full flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-4 sm:gap-8">
                        ...
                        {/* Mobile Menu Toggle */}
                        <button
                            className="lg:hidden w-10 h-10 flex flex-col items-center justify-center gap-1.5 relative z-50 mix-blend-difference"
                            onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
                        >
                            <span className={`block w-6 h-0.5 bg-white transition-transform duration-300 ${isMobileMenuOpen ? 'rotate-45 translate-y-2' : ''}`} />
                            <span className={`block w-6 h-0.5 bg-white transition-opacity duration-300 ${isMobileMenuOpen ? 'opacity-0' : ''}`} />
                            <span className={`block w-6 h-0.5 bg-white transition-transform duration-300 ${isMobileMenuOpen ? '-rotate-45 -translate-y-2' : ''}`} />
                        </button>
                    </div>
                </div>
            ...
            <AnimatePresence>
                {isMobileMenuOpen && (
                    <motion.div
                        ...
                        className="fixed inset-0 z-[90] bg-[#02020a]/98 backdrop-blur-3xl overflow-y-auto pt-28 pb-10 px-4 sm:px-10 flex flex-col items-center justify-start min-h-screen"
                    >
                        <nav className="flex flex-col items-center gap-6 sm:gap-8 w-full">
                            {['home', 'about', 'skills', 'portfolio', 'education', 'contact'].map(key => (
                                <button
                                    key={key}
                                    onClick={() => scrollToSection(key)}
                                    className={`text-sm sm:text-xl font-black uppercase tracking-[0.4em] transition-all w-full text-center py-2 ${activeSection === key ? 'opacity-100 scale-110' : 'opacity-50 hover:opacity-100'}`}
                                    style={{ color: activeSection === key ? THEMES[currentTheme].primary : 'white' }}
                                >
                                    {key}
                                </button>
                            ...
                            <button
                                className="mt-6 sm:mt-8 flex items-center justify-center gap-3 w-full max-w-[250px] px-8 py-3.5 sm:py-4 rounded-full border text-[10px] sm:text-xs font-black uppercase tracking-widest shadow-lg"
                                style={{
                                    borderColor: `${THEMES[currentTheme].primary}40`,
                                    color: THEMES[currentTheme].primary,
                                    backgroundColor: `${THEMES[currentTheme].primary}10`
            ...
            <main className="relative z-10 pt-20">
                ...
                <section id="home" className="min-h-screen flex flex-col justify-center">
                    <motion.div
                        ...
                        className="max-w-[1600px] mx-auto px-4 sm:px-10 py-6 sm:py-24 w-full"
                    >
                        <motion.div variants={{
                            ...
                            <CinematicShineBorder theme={currentTheme}>
                                <div className="space-y-8 sm:space-y-16">
                                    <motion.div
                                        ...
                                        className="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left gap-6 lg:gap-12"
                                    >
                                        <div className={`w-24 h-24 sm:w-32 sm:h-32 lg:w-40 lg:h-40 rounded-full overflow-hidden border-4 transition-all duration-1000 shadow-[0_0_80px_rgba(139,92,246,0.3)] shrink-0`} style={{ borderColor: `${THEMES[currentTheme].primary}40`, boxShadow: `0 0 80px ${THEMES[currentTheme].primary}30` }}>
                                            <img src="/assets/images/team/portfolio/ibnay_img.jpeg" className="w-full h-full object-cover" />
                                        </div>
                                        <div className="space-y-4">
                                            ...
                                            <h2 className="text-3xl sm:text-4xl lg:text-6xl font-black text-white leading-tight tracking-tighter italic font-serif">
                                    ...
                                    <div className="space-y-10">
                                        ...
                                        >
                                            {[
                                                { name: 'Inertia', icon: 'https://cdn.simpleicons.org/inertia/9553E9' },
                                                ...
                                                <div
                                                    key={tech.name}
                                                    className={`px-4 sm:px-5 py-1.5 sm:py-2 border rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all duration-1000 shadow-[0_0_20px_rgba(0,0,0,0.1)] flex items-center gap-2 group/tech cursor-pointer`}
                                                    style={{
                                                ...
                                                >
                                                    {tech.icon && <img src={tech.icon} alt={tech.name} className="w-3 h-3 sm:w-4 sm:h-4 object-contain filter grayscale opacity-60 group-hover/tech:grayscale-0 group-hover/tech:opacity-100 transition-all duration-500" />}
                                                    {tech.name}
                                                </div>
                                            ))}
                                        </motion.div>
                                        <div className="space-y-6">
                                            <motion.h1
                                                ...
                                                className="text-3xl sm:text-5xl lg:text-6xl font-black text-white tracking-tighter leading-tight text-center sm:text-left"
                                            >
                                            ...
                                            <motion.p
                                                ...
                                                className="text-gray-400 text-lg sm:text-xl lg:text-2xl max-w-4xl font-light leading-relaxed text-center sm:text-left mx-auto sm:mx-0"
                                            >
                ...
                <section id="about" className="py-16 sm:py-24">
                    <motion.div
                        ...
                        className="max-w-8xl mx-auto px-4 sm:px-10"
                    >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-12 lg:gap-16">
                            {PROJECTS_DATA.map((project, idx) => (
                                <motion.div
                                    ...
                                    viewport={{ once: true }}
                                    className="group bg-[#02020a]/60 backdrop-blur-3xl rounded-[2.5rem] p-5 sm:p-8 lg:p-10 border transition-all duration-700 hover:bg-[#02020a]/80"
                                ...
                                >
                                    <div className="space-y-6 sm:space-y-10">
                                        <div className="aspect-video relative rounded-2xl overflow-hidden bg-white/5 border border-white/10 group-hover:border-white/20 transition-all duration-700">
                                            <img
                                                src={project.image}
                                                className="w-full h-auto object-cover grayscale-0 opacity-100 md:grayscale md:opacity-60 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-700 md:duration-[5000ms] ease-in-out absolute top-0 left-0 hover-scroll-img"
                                            />
                                        ...
                                        <div className="space-y-6 px-2">
                                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-0">
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {project.tech.slice(0, 3).map(t => (
                                                        <span key={t} className="text-[8px] text-white/30 uppercase tracking-widest border border-white/5 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full bg-white/[0.02]">
                                                            {t}
                                                        </span>
                                                    ))}
                                                </div>
                                            ...
                                            <div className="space-y-3 sm:space-y-4">
                                                <h4 className="text-2xl sm:text-3xl font-black text-white group-hover:translate-x-2 transition-transform duration-500 tracking-tighter">
                                                    {project.title}
                                                </h4>
                ...
                {/* Contact Section */}
                <section id="contact" className="py-16 sm:py-24 pb-32 sm:pb-48">
                    <motion.div
                        initial="hidden"
                        whileInView="visible"
                        viewport={{ once: true, margin: "-100px" }}
